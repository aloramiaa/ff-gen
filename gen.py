...
if __name__ == "__main__":
    try:
        parser = argparse.ArgumentParser(description='BIGBULL Account Generator')
        parser.add_argument('--mode', choices=['interactive', 'cli'], default='interactive', help='Run mode: interactive (default) or cli')
        parser.add_argument('--region', help='Target region (e.g., IND, BR, ME) or GHOST')
        parser.add_argument('--count', type=int, help='Number of accounts to generate')
        parser.add_argument('--name-prefix', help='Account name prefix')
        ...
        if install_requirements():
            if args.mode == 'cli' or (args.region and args.count and args.name_prefix and args.password_prefix):
                # CLI Mode
                if not all([args.region, args.count, args.name_prefix, args.password_prefix]):
                    print_error("Missing required arguments for CLI mode: --region, --count, --name-prefix, --password-prefix")
                    sys.exit(1)
                
                is_ghost = False
                selected_region = args.region.upper()
        ...
        # Only restart in interactive mode
        if len(sys.argv) == 1:
